...
function whatIsTheHandType(hand: string) {
  ...
  else throw Error(`this hand is weird: ${hand}`);
}
...
function getResult(input: string[]) {
  ...
  partOneResult = cleanedInput.reduce((prev, current, index) => {
...
const partOneExpected = 6440;
const { partOneResult } = getResult(example);
